...
import { FlaskConical, TreePine, CalendarDays, Coins, BookOpen, Leaf, User, AlertTriangle, ShieldCheck, ArrowRight, ExternalLink, ChevronLeft, ChevronRight, Mail, Phone, ClipboardCheck, MapPinned, Sparkles, CloudRain, Droplets, Sprout, Sun, Wind, MessageSquare, MessageCircle, HeartHandshake, Check, Smartphone, Download, MoreHorizontal } from "lucide-react"
...
function AppFooter({ onContactClick }: { onContactClick: () => void }) {
  return (
    <footer className="relative z-10 mt-auto border-t border-border/70 bg-card/75 px-4 py-4 text-center backdrop-blur-md sm:py-5">
...
export default function AppShell() {
  ...
  const mobilePrimaryTabs = useMemo(
    () => MOBILE_TABS.filter(tab => visibleTabs.some(visibleTab => visibleTab.id === tab.id)),
    [visibleTabs],
  )
  const mobileOverflowTabs = useMemo(
    () => visibleTabs.filter(tab => !MOBILE_TABS.some(primaryTab => primaryTab.id === tab.id)),
    [visibleTabs],
  )
  const isMobileOverflowActive = mobileOverflowTabs.some(tab => tab.id === activeTab)
  ...
  if (!user) {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col guest-layout-container">
        ...
        <AnimatedBackground />
        <header className="app-shell-header sticky top-0 z-40 border-b border-border/80 bg-card/95 px-4 py-3 backdrop-blur-xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
            <button
              onClick={() => setActiveTab("dashboard")}
              className="flex min-w-0 items-center gap-2.5 rounded-xl text-left active:scale-95"
            >
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-sm ring-1 ring-primary/20">
  ...
  return (
    <div className="h-screen bg-transparent flex flex-col relative overflow-hidden app-shell-container">
      ...
      <AnimatedBackground />
      {/* Top Header Bar */}
      <header className="app-shell-header sticky top-0 z-40 bg-card/95 px-3 sm:px-4 md:px-8 py-2.5 flex items-center justify-between gap-2 shrink-0 border-b border-border/80 backdrop-blur-xl overflow-hidden">
        <button
          onClick={() => setActiveTab("dashboard")}
          className="relative flex min-w-0 items-center gap-2 rounded-xl sm:gap-3 hover:opacity-90 transition-opacity active:scale-95"
        >
          <div className="shrink-0 p-2 sm:p-2.5 bg-primary text-primary-foreground rounded-xl shadow-sm ring-1 ring-primary/20">
      ...
      <div className="relative flex min-w-0 flex-1 overflow-hidden">
        {/* Desktop Sidebar */}
        <nav aria-label="เมนูหลัก" className="app-shell-sidebar hidden lg:flex flex-col w-60 bg-card/80 border-r border-border/80 py-5 px-3 gap-1.5 shrink-0 relative overflow-hidden backdrop-blur-xl">
        ...
        <main id="main-content" tabIndex={-1} className="min-w-0 flex-1 overflow-y-auto bg-transparent pb-24 lg:pb-0 flex flex-col">
          <div
      ...
      <nav aria-label="เมนูมือถือ" className="mobile-bottom-nav lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-card/95 px-2 py-2 flex items-center gap-1 w-full sm:max-w-xl sm:left-1/2 sm:-translate-x-1/2 sm:mb-3 sm:rounded-2xl sm:border sm:border-border border-t border-border safe-area-bottom backdrop-blur-xl shadow-[0_-10px_30px_rgba(20,83,45,0.10)]">
        {mobilePrimaryTabs.map(tab => {
          ...
          return (
            <button
              ...
              className={`min-w-0 flex-1 flex flex-col items-center justify-center gap-0.5 py-2 rounded-xl transition-all ${isActive ? "bg-primary text-primary-foreground shadow-sm" : "text-foreground/70 dark:text-foreground/60 hover:bg-primary/8"
                }`}
            >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                aria-current={isMobileOverflowActive ? "page" : undefined}
                className={`min-w-0 flex-1 flex flex-col items-center justify-center gap-0.5 rounded-xl py-2 transition-all ${isMobileOverflowActive ? "bg-primary text-primary-foreground shadow-sm" : "text-foreground/70 hover:bg-primary/8"}`}
              >
                <MoreHorizontal size={20} className="shrink-0" />
                <span className="max-w-full truncate px-1 text-[10px] font-semibold min-[360px]:text-xs">เพิ่มเติม</span>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="top" align="end" sideOffset={12} className="w-52 rounded-2xl p-2">
              {mobileOverflowTabs.map(tab => {
                ...
                return (
                  <DropdownMenuItem
                    ...
                    className={`gap-3 rounded-xl px-3 py-3 font-bold ${activeTab === tab.id ? "bg-primary/10 text-primary" : ""}`}
                  >
                    <Icon size={18} />
                    {tab.label}
                  </DropdownMenuItem>
